<?php
namespace Mirai\Payment\Epay\V1;
...
class EpayCore
{
	...
	public function apiPay($param_tmp){
		...
		if ($response === 'success') {
			return ['code' => -1, 'msg' => 'API返回格式错误(纯文本success)，请检查易支付接口URL是否正确', 'raw' => $response];
		}
		$arr = json_decode($response, true);
		if (!is_array($arr)) {
			return ['code' => -1, 'msg' => 'API返回格式错误: ' . mb_substr($response, 0, 200), 'raw' => $response];
		}
		if (!isset($arr['code'])) {
			$arr['code'] = -1;
			$arr['raw'] = $response;
	...
	public function verifyNotify(){
		if(empty($_GET) || !isset($_GET['sign'])) return false;

		$params = $_GET;
		unset($params['mirai_api'], $params['gateway']);
